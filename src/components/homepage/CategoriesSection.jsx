  ...
  return (
    <section className='w-4/5 mx-auto mt-4 lg:mt-20'>
      <div className='hidden lg:flex items-center gap-2'>
        ...
        <p className='text-[#DB4444] font-semibold'>{language === 'en' ? 'Categories' : 'Kategori'}</p>
      </div>
      <div className='flex justify-between items-center mt-2'>
        <h2 className='text-sm lg:text-4xl font-semibold'>{language === 'en' ? 'Browse By Category' : 'Telusuri Berdasarkan Kategori'}</h2>
        <div className='hidden lg:flex gap-2'>
          <button className='custom-prev-button rounded-full bg-gray-100 p-2 disabled:opacity-30'>
            <img src={Left} alt='Left' />
          </button>
          <button className='custom-next-button rounded-full bg-gray-100 p-2 disabled:opacity-30'>
      ...
      <div className='flex overflow-x-auto gap-4 mt-4 lg:hidden no-scrollbar'>
        {categories.map((category, index) => (
          <Link key={index} to='/products/category/electronics' className='flex flex-col items-center flex-shrink-0'>
